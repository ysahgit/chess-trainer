...
3. Click any filled repertoire slot to start a training session immediately.
...
Eight repertoire slots are shown in a grid. Any slot can be empty or pre-filled with a repertoire.
...
- **⬇ Export** (top-right of the logo bar) → downloads a self-contained `chess-trainer.html` with all repertoires and settings baked in.
...
Reached by tapping ⚙ on a filled slot or + on an empty slot.
...
- The repertoire title (from the `//` header line in the PGN file) is shown below the slot name.
...
- The status bar shows the current line number and its per-line name prefixed with 🎯.
- Move annotations in `{curly braces}` override the line name in the status bar as each move is reached.
- **‹ ›** step back and forward within the current line. Stepping back marks the line as having an error.
- **⟳** flips the board and switches the active color — if it's now the opponent's turn, their move plays automatically.
...
Line name    1. d4 d5   2. Nc3 Nf6   3. Bf4 Nc6
Another line 1. d4 d5   2. Nc3 c5    3. e3 Nc6
```

- The `//` line is the **repertoire title**, shown in the active session panel.
- Each subsequent line contains the **line name** followed by the moves, separated by whitespace or tabs.
- Leading emoji in line names are stripped on import and replaced with 🎯 in the status bar during training.
- The file should have no emoji in it — the app adds them automatically.
...
| Built-in repertoires (slots 1–8) | Hardcoded in `BUILTIN_REPS` inside `index.html` |
...
> **Note:** Export requires the file to be served over `http://` (e.g. `python3 -m http.server 8080`). It will not work when opened directly as a `file://` URL.

---

## Slots

All eight slots use uniform IDs `slot1` through `slot8`. Any slot can be empty or pre-filled. There is no special-casing between slots — all behave identically.

The current built-in repertoires are:

| Slot | Name | Color |
|---|---|---|
| slot1 | Jobava main lines | White |
| slot2 | Jobava side lines | White |
| slot3 | Scandi-Nf6 | Black |
| slot4 | QGA | Black |
| slot5 | QGD | Black |
| slot6 | CK-Defence | Black |
| slot7 | *(configurable)* | — |
| slot8 | *(configurable)* | — |
...
| Landscape (≤ 900 px, wider than tall) | 160 px sidebar on the right; board fills the rest. |
